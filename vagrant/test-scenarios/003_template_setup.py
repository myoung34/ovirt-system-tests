...
TEMPLATE_CENTOS7 = 'CentOS_7_Generic_Cloud_Image_v1608_for_x86_64'

VM0_NAME = 'vm0'
VM1_NAME = 'vm1'
DISK0_NAME = '%s_disk0' % VM0_NAME
DISK1_NAME = '%s_disk1' % VM1_NAME
GLANCE_DISK_NAME = 'CentOS_7_Generic_Cloud_Image_v1608_for_x86_64_glance_disk'
...
def add_vm_template(api):
    ...
    if api.templates.get(name=TEMPLATE_CENTOS7) is None:
        raise SkipTest('%s: template %s not available.' % (add_vm_template.__name__, TEMPLATE_CENTOS7))

    vm_memory = 512 * MB
    vm_params = params.VM(
        name=VM1_NAME,
        description='CentOS imported from Glance as Template',
        memory=vm_memory,
        cluster=params.Cluster(
            name=TEST_CLUSTER,
        ),
...
def template_export(api):
    template_centos = api.templates.get(TEMPLATE_CENTOS7)

    if template_centos is None:
    ...
    template_centos.export(
        params.Action(
            storage_domain=api.storagedomains.get(SD_TEMPLATES_NAME)
         )
    )
    testlib.assert_true_within_long(
        lambda: api.templates.get(TEMPLATE_CENTOS7).status.state == 'ok',
    )
