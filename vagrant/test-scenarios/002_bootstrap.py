...
GLANCE_AVAIL = False
CENTOS_IMAGE_NAME = 'CentOS 7 Generic Cloud Image v1608 for x86_64'
...
def add_hosts_3(prefix):
    ...
    for host in hosts:
        testlib.assert_true_within(_host_is_up, timeout=45 * 60)
...
def add_hosts_4(prefix):
    ...
    for api_host in api_hosts:
        testlib.assert_true_within(_host_is_up_4, timeout=45*60)
    ...
    if MASTER_SD_TYPE == 'iscsi':
        vt = utils.VectorThread(
            [
               ...
               #[12/07/2017] commeting out iso domain creation until we know why it causing random failures
...
def generic_import_from_glance(api, image_name=CENTOS_IMAGE_NAME, as_template=False, image_ext='_glance_disk', template_ext='_glance_template', dest_storage_domain=MASTER_SD_TYPE, dest_cluster=CLUSTER_NAME):
    ...
    print('marc: {}'.format(template_name))
...
def import_template_from_glance(prefix):
    ...
    generic_import_from_glance(api, image_name=CENTOS_IMAGE_NAME, image_ext='_glance_template', as_template=True)
